Module Module1
    Dim data As String = "8C TS KC 9H 4S 7D 2S 5D 3S AC
...
AS KD 3D JD 8H 7C 8C 5C QD 6C"
    Class CardReturn
        Public Kickers As SortedSet(Of Integer)
        Public TwoPairCard2 As Integer
        Public BestInSet As Integer
        Private _val As Integer = -100
        Public Property HandValue() As Integer
            Get
                Return _val
            End Get
            Set(ByVal value As Integer)
                _val = Math.Max(_val, value)
            End Set
        End Property
    ...
    Class Card
        ...
        Public Sub New(v As Char, s As Char)
            Suit = s
            Select Case v
                Case "T"
                    Value = 10
        ...
        End Function
    End Class
    Public Function RateHand(x As String) As CardReturn
        Dim r As CardReturn = New CardReturn

        'TJQKA
        'HCDS
        Dim cards(4) As Card
        Dim idx As Integer = 0
        For Each cs As String In x.Split(" ")
            cards(idx) = New Card(cs(0), cs(1))
            idx += 1
        Next
        Array.Sort(cards)
        'if all cards are the same suit, at least flush, not full house or 4 of kind so check for straight and royal
        Dim firstSuit As Char = cards.First.Suit
        Dim allSameSuit As Boolean = True
        Dim firstValue As Integer = cards.First.Value
        Dim largestValue As Integer = 0
        Dim straight As Boolean = True
        r.Kickers = New SortedSet(Of Integer)
        For Each c As Card In cards

            r.Kickers.Add(c.Value)

            largestValue = Math.Max(largestValue, c.Value)
            If c.Suit <> firstSuit Then
                allSameSuit = False
            End If
            If Not (c.Value = firstValue Or (c.Value = 1 And firstValue = 13)) Then
                straight = False
            End If
            firstValue += 1
        Next
        ...
        If allSameSuit Then
            If straight Then
                If cards.First.Value = 10 Then
                    r.HandValue = 10 'ROYAL FLUSH
                Else
                    r.HandValue = 9 'STRAIGHT FLUSH
        ...
        End If
        If straight And Not allSameSuit Then
            r.HandValue = 5 'STRAIGHT
        End If

        Dim valueCounts As List(Of Integer) = New List(Of Integer)
        For xd As Integer = 0 To 14
            valueCounts.Add(0)
        Next
        For Each c As Card In cards
            valueCounts(c.Value) += 1
        Next
        If valueCounts.Max = 4 Then
            r.BestInSet = valueCounts.IndexOf(4)
            r.HandValue = 8 '4 OF A KIND
        End If
        If valueCounts.Max = 3 Then
            r.BestInSet = valueCounts.IndexOf(3)

            If valueCounts.Contains(2) Then
                r.HandValue = 7 'FULL HOUSE
                r.TwoPairCard2 = valueCounts.Find(Function(v) v = 2)
            Else
                r.HandValue = 4 '3 OF A KIND

            End If

        End If
        If valueCounts.Max = 2 Then

            r.BestInSet = valueCounts.FindAll(Function(v) v = 2).Max
            Console.WriteLine("Best = " & r.BestInSet)

            If valueCounts.FindAll(Function(v) v = 2).Count = 2 Then
                r.TwoPairCard2 = valueCounts.FindAll(Function(v) v = 2).Min
                r.HandValue = 3 '2 PAIR
            Else
                r.HandValue = 2 '1 PAIR

            End If
            For xd As Integer = 1 To 14
                If valueCounts(xd) = 2 Then
                    If xd > r.BestInSet Then
                        If r.HandValue = 3 Then

                            r.TwoPairCard2 = r.BestInSet
                        End If
                        r.BestInSet = xd
                    ElseIf r.HandValue = 3 Then
                        r.TwoPairCard2 = xd
                    End If
                End If
            Next
        ...
        r.HandValue = largestValue - 13


        r.Kickers.RemoveWhere(Function(v) v = r.BestInSet Or v = r.TwoPairCard2)

        Return r

    End Function

    Public Function GetWinner(h1 As CardReturn, h2 As CardReturn)
        If h1.HandValue > h2.HandValue Then
            Return True
        ElseIf h1.HandValue < h2.HandValue Then
            Return False
        End If
        Console.WriteLine("Going to Set")
        If h1.BestInSet > h2.BestInSet Then
            Return True
        ElseIf h1.BestInSet < h2.BestInSet Then
            Return False
        End If
        Console.WriteLine("Going to Card")

        'win by 2nd card?
        If h1.TwoPairCard2 > h2.TwoPairCard2 Then
            Return True
        ElseIf h1.TwoPairCard2 < h2.TwoPairCard2 Then
            Return False
        End If
        Console.WriteLine("Going to kicker")
        'win by kickers
        Dim idx As Integer = h1.Kickers.Count - 1
        While h1.Kickers(idx) = h2.Kickers(idx)
            Console.WriteLine("Another Kicker")
            idx -= 1
        End While
        If h1.Kickers(idx) > h2.Kickers(idx) Then
            Return True
        Else
            Return False
        End If
    ...
    Sub Main()
        ...
        Dim OT As Integer = 0
        For Each game As String In data.Split(vbLf)
            'Console.WriteLine("This Game:" & game)
            Dim p1 As String = game.Substring(0, 14)
            Dim p2 As String = game.Substring(15, 14)
            Dim h1 As CardReturn = RateHand(p1)
            Dim h2 As CardReturn = RateHand(p2)
            Console.WriteLine(p1 & " " & RateHand(p1).HandValue & " " & p2 & " " & RateHand(p2).HandValue)
            If GetWinner(h1, h2) Then
                total += 1
            Else
                OT += 1
            End If
        ...
        Next
        Console.WriteLine(total & " " & total + OT)
        Console.ReadLine()
    End Sub
